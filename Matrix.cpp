...
double Matrix::min_element() {
    double_t min_ele = MAXFLOAT;
    for (uint32_t i = 0 ; i != rows; ++i) {
        for (uint32_t j = 0 ; j != cols ; ++j)
            if (this->operator()(i, j) < min_ele) min_ele = this->operator()(i,j);
    }
    return min_ele;
}

double Matrix::max_element() {
    double_t max_ele = -MAXFLOAT;
    for (uint32_t i = 0 ; i != rows; ++i) {
        for (uint32_t j = 0 ; j != cols ; ++j)
            if (this->operator()(i, j) > max_ele) max_ele = this->operator()(i,j);
    }
    return max_ele;
}
